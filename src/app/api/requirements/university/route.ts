...
export async function GET() {
    try {
        ...
        if (error) {
            console.error('Error fetching university requirements:', error);
        ...
        console.error('Error in university requirements API:', error);
        ...
        return NextResponse.json(
            ApiErrorSchema.parse({ error: 'Failed to fetch university requirements' }),
            { status: 500 }
        );
    }
} 